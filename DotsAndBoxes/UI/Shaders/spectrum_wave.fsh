...
float SHEET_THICKNESS = 0.012;
float NOISINESS = 10.0;

vec4 INNER_COLOR = vec4(0.0, 30.0, 30.0, 1.0);
vec4 OUTER_COLOR = vec4(20.0, 20.0, 30.0, 1.0);
...
void main( void )
{
    vec2 center = (u_sprite_size) / length(u_sprite_size) / 1.3;
    vec2 uv = (v_tex_coord.xy / center.x*0.55) * 2.0 - vec2(center.x / center.y, 1.0);
